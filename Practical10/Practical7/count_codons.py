...
# Input validation 
print("Please input termination codon (TAA/TAG/TGA):")
target = input().strip().upper()
while target not in ["TAA", "TAG", "TGA"]:
    print("Invalid input! Please enter TAA/TAG/TGA:")
    target = input().strip().upper()
...
print(f"\n{target} upstream codon count: {total}")
print("Top 10 most common codons:")
for c, n in count.most_common(10):
    print(f"{c}: {n}")

# Draw pie chart (frequency > 1%)
labels = []
sizes = []
